...
pub fn m4_scalar_with_x<Tx, Ty>(x: &[Tx], arr: &[Ty], n_out: usize) -> Vec<usize>
where
...
pub fn m4_scalar_without_x<T: Copy + PartialOrd>(arr: &[T], n_out: usize) -> Vec<usize>
where
    for<'a> &'a [T]: ArgMinMax,
{
    assert_eq!(n_out % 4, 0);
...
mod tests {
    ...
    fn test_m4_scalar_without_x_correct() {
        ...
        let sampled_indices = m4_scalar_without_x(&arr, 12);
    ...
    fn test_m4_scalar_without_x_parallel_correct(n_threads: usize) {
        ...
        let sampled_indices = m4_scalar_without_x_parallel(&arr, 12, n_threads);
        let sampled_values = sampled_indices
            .iter()
            .map(|x| arr[*x])
            .collect::<Vec<f32>>();
    ...
    fn test_m4_scalar_with_x_correct() {
        ...
        let sampled_indices = m4_scalar_with_x(&x, &arr, 12);
        let sampled_values = sampled_indices
            .iter()
            .map(|x| arr[*x])
            .collect::<Vec<f32>>();
    ...
    fn test_m4_scalar_with_x_parallel_correct(n_threads: usize) {
        ...
        let sampled_indices = m4_scalar_with_x_parallel(&x, &arr, 12, n_threads);
        let sampled_values = sampled_indices
            .iter()
            .map(|x| arr[*x])
            .collect::<Vec<f32>>();
    ...
    fn test_m4_scalar_with_x_gap() {
        ...
        let sampled_indices = m4_scalar_with_x(&x, &arr, 20);
    ...
    fn test_m4_scalar_with_x_gap_parallel(n_threads: usize) {
        ...
        let sampled_indices = m4_scalar_with_x_parallel(&x, &arr, 20, n_threads);
        assert_eq!(sampled_indices.len(), 16); // One full gap
        let expected_indices = vec![0, 0, 29, 29, 30, 30, 50, 50, 51, 51, 69, 69, 70, 70, 99, 99];
        assert_eq!(sampled_indices, expected_indices);
    ...
    fn test_many_random_runs_correct(n_threads: usize) {
        ...
        for _ in 0..100 {
            ...
            let idxs1 = m4_scalar_without_x(arr.as_slice(), N_OUT);
            let idxs2 = m4_scalar_with_x(&x, arr.as_slice(), N_OUT);
            assert_eq!(idxs1, idxs2);
            let idxs3 = m4_scalar_without_x_parallel(arr.as_slice(), N_OUT, n_threads);
            let idxs4 = m4_scalar_with_x_parallel(&x, arr.as_slice(), N_OUT, n_threads);
